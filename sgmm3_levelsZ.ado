...
mat colnames uu = `varlist'
* "gc gdp ca reer"
*`nameu'     
mat rownames uu = `varlist'
 *"gc gdp ca reer"
**mat list uu
*
*di " "
...
/* Para el ZZ  */
/*  county = 0
ZZ = J(cols(Z),cols(Z),0)
for(i=1;i<= numPais;i++){
	Zi = Z[county+1..county+pTT[i],.]
	ZiZi = Zi'*Zi
	ZZ = ZZ + ZiZi
	county = county + pTT[i]
}
ZZ = invsym(1/numobs * ZZ)
printf("done with... ZuuZ... \n")
	...
	/*    ZZ = 1/numobs * ZZ	*/
/*  ZZ = qrinv(cross(Z,Z))   */
printf("processing... ZtXtZZZtX = invsym(ZtX'* ZZ * ZtX ) ... \n")
ZtXtZZZtX = invsym(ZtX'* ZZ * ZtX )
/*   ZtXtZZZtX = qrinv(ZtX'* ZZ * ZtX )      */
printf("processing... b2sls = ZtXtZZZtX * ZtX' * ZZ * ZtY... \n")
...
ZuuZ = invsym( 1/numobs *cross(ZU, ZU))
...
uZ = u'*Z
ZuuZ = invsym( 1/numPais *cross(uZ,uZ))   
...
printf("processing... ZtXtZuuZZtX = invsym(ZtX'* ZuuZ * ZtX )... \n")
ZtXtZuuZZtX = invsym(ZtX'* ZuuZ * ZtX )
printf("processing... b = ZtXtZuuZZtX * ZtX' * ZuuZ * ZtY... \n")
